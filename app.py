...
if option == 'Stock Predictions':
    stock_name_input = st.multiselect('Enter Stock Name', names, [], key="stock_input")

    # Find the corresponding symbol for the entered stock name
    if stock_name_input:
        stock_symbols = [symbols[names.index(name)] for name in stock_name_input]
        st.write("Found stock symbols:", stock_symbols)
    else:
        st.write("No stock selected.")

    for stock_symbol in stock_symbols:
        start_date = st.date_input('Start Date', pd.Timestamp('2023-01-01'), key=f"start_date_{stock_symbol}")
        end_date = st.date_input('End Date', pd.Timestamp('2024-04-30'), key=f"end_date_{stock_symbol}")
    ...
    stock_name_input = st.multiselect('Enter Stock Name', names, [], key="stock_input_sentiment")
    ...
    for stock_symbol in stock_symbols:
        start_date = st.date_input('Start Date', pd.to_datetime('2023-01-01'), key=f"start_date_sentiment_{stock_symbol}")
        end_date = st.date_input('End Date', pd.to_datetime('2024-04-30'), key=f"end_date_sentiment_{stock_symbol}")
