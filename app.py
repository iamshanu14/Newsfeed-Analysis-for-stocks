...
if option == 'Stock Predictions':
    stock_name_input = st.text_input('Enter Stock Name', '').strip().lower()

    # Find the corresponding symbol for the entered stock name
    if stock_name_input:
        matches = [name for name in names if stock_name_input in name.lower()]

        if matches:
            stock_symbol = symbols[names.index(matches[0])]
